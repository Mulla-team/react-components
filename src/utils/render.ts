...
TBag > (props : Props < TTag, TBag, any >, bag : TBag, tag : React.ElementType) {
  const {
    as: Component = tag,
    children,
    ...passThroughProps
    ...
    if (Object.keys(passThroughProps).length > 0) {
      if (Array.isArray(resolvedChildren) && resolvedChildren.length > 1) {
        ...
        if (Error.captureStackTrace) 
          Error.captureStackTrace(err, render)
        throw err
      }

      if (!React.isValidElement(resolvedChildren)) {
        const err = new Error(`You should render an element as a child. Did you forget the as="..." prop?`)
        if (Error.captureStackTrace) 
          Error.captureStackTrace(err, render)
        throw err
      }
 ...
 */
function mergeEventFunctions(passThroughProps : Record < string, any >, existingProps : Record < string, any >, functionsToMerge : string[]) {
...
function compact < T extends Record < any,
any >> (object : T) {
